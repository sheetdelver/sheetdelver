...
export async function run(): Promise<void> {
    ...
    try {
        ...
        assert.equal(record.reason?.includes('Contract module-api 1.2.0 does not satisfy constraint >=2.0.0'), true);
        ...
        assert.equal(record.validation?.providedApiContracts?.['module-api'], '1.2.0');
        assert.equal(

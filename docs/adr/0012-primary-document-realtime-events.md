...
**June 6, 2026 — Embedded-child create must be idempotent (real-world fix).** Firing rule #1 ("emit only on observable change") guarantees one *event* per logical mutation when a write is applied twice — once by the Repository mirror, once by the broadcast (see "Event Sequence for a Sheet-Delver-Initiated Write"). But idempotency also has to hold for the *apply itself*. Top-level docs are stored in a `Map` keyed by `_id`, so a double create is a self-correcting `set`. **Embedded children are stored in arrays, and every store appended on create unconditionally** (`actor.items.push(...)`, `combat.combatants.push(...)`, cards/sounds/pages/results/effects) — so a single added child became *two* rows when the mirror and the broadcast both applied (surfaced when a module created an actor item: it appeared twice). Fixed by routing every embedded create through a shared `appendCreatedById(existing, created)` helper in `PrimaryDocumentStore` that skips children already present by `_id` (children with no id are appended — no key to dedupe on). The `update`/`delete` paths were already `_id`-keyed and idempotent; this brings `create` in line. Hardened across all embedded-child stores: Actor (items + effects), Item (effects), Combat (combatants — see ADR-0028), Cards (cards), Playlist (sounds), Journal (pages), RollTable (results). Covered by a helper unit test (`primary-document-base.test.ts`) and a store-integration regression (`actors/actor-store.test.ts`).

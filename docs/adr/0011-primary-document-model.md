...
- [X] Extend `CoreSocket.fetchByUuid` world-doc short-circuit to `RollTable`, `Macro`, `Playlist`, `Cards` — mirrors the Actor/Item branches, fails closed with `PrimaryDocumentCacheNotReadyError` if the Store isn't seeded. Compendium UUIDs keep the existing pack-fetch path. The original ADR-0011 compendium follow-up is now superseded by ADR-0015's Pathway B shard lookup and parsed pack-document fallback prep.
  Files: `src/server/core/foundry/sockets/CoreSocket.ts`.

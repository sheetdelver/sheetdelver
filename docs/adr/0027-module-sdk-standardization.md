...
5. **Singleton services; `ModuleRuntime` is the per-request handle over them.** The platform mounts the document/roll/table services + stores **once** at server start. What a module receives is a thin `ModuleRuntime` handle that forwards to those singletons — the services are never re-instantiated per request and module code never calls `getInstance()` (internal wiring). The runtime comes in **two flavors**:
   ...
   **Amendment (2026-06-08 — audit correction).** The implementation satisfied the normal player path (module writes dispatch through the user's `ClientSocket`, which refuses to fall back to `CoreSocket`), but the module API boundary still accepted the app service token because `/api/modules/*` uses permissive auth before global route auth. That could bind `req.runtime` to a system-backed `RouteFoundryClient` for any holder of the service token, which contradicts the "acting user's Foundry socket" invariant even though the token is not client-issued. The fix is to require a real `req.userSession` before loading or invoking a module API handler, and to deny the request if `req.userSession.userId` does not match the bound route client's `userId`. Module UI/assets remain permissive where designed; module API handlers are user-bound only.
...
8. **Static `apiRoutes`; the runtime arrives on the request (`req.runtime`).** No per-request factory. A module exports a static handler table once; the platform attaches the per-request runtime to the request before invoking the handler.
   ...
   **Amendment (2026-06-08 — dispatch boundary hardening).** `ModuleProxyService` now treats the user session as mandatory input, not a value that can be reconstructed from `rawClient.userId`. This closes the gap where a non-user principal could still receive a module `req.runtime` because it had a `RouteFoundryClient`. Missing Foundry clients still return `401`; system/service-token or otherwise non-user module API calls return `403`; mismatched session/client identities return `403` before registry lookup or handler invocation.
...
9. **Default to the calling user; explicit `{ access }` only for the non-norm case.** Document ops on `req.runtime` default their acting identity to `req.userSession` — so `req.runtime.documents.patch('Actor', id, updates)` acts as the caller with **no ceremony**. A module passes `{ access }` (and optional `minOwnership`) **only** to act as a different subject (e.g. system context) — the deliberate, visible escape hatch. Ownership thresholds use the Foundry-style ladder (`limited` / `observer` / `owner`); `getAccessContext()` reads the caller's access or builds an override.
   ...
   **Amendment (2026-06-08 — access override correction).** The original text overpromised the `{ access }` escape hatch for writes. The implementation could change the local ownership subject, but dispatch still used the request-bound Foundry transport, so it did not truly "act as" another user or system principal. Writes now reject `{ access }` when `access.userId` differs from the bound transport user. Reads may still evaluate visibility through an explicit subject, but alternate-principal writes require a future audited transport resolver rather than a local subject override.
...
10. **Enforcement at the service/store boundary, fail-closed.** The platform already declares a security model — `ModuleTrustDeclaration` (`trust.tier`) and `ModulePermissionDeclaration` (`network.outbound`, `filesystem`, `adminRoutes`, `sensitiveData`) — and a live policy layer (`src/modules/registry/security/permissionPolicy.ts`, `trustPolicy.ts`). The SDK exports the declarations but does not yet gate any surface on them. The document service must be wired to this policy, with scoping riding the store ownership layer (`src/server/core/documents/primary/base/ownership.ts`):
    ...
    **Amendment (2026-06-08 — Foundry permission model correction).** The "module trust/permission grants" phrasing above was too broad for document CRUD. Foundry does not have a `permissions.sensitiveData` document-write gate; document mutation authority is the acting Foundry user and Foundry's own permission/ownership checks on the socket operation. Sheet Delver keeps a local fail-closed preflight (readiness, subject resolution, visible/writeable ownership, and per-op `commit` verification), but it is not a replacement for Foundry permissioning and it does not use `sensitiveData` as a write gate. Module trust/permission declarations remain lifecycle/install/admin policy surfaces unless a later ADR assigns them to a specific runtime capability.
...
23. **Capability detection.** `SDK.capabilities.supports('combat' | 'effects' | 'tables' | …): boolean` for optional surfaces, paired with the existing `SDK_VERSION` / `API_CONTRACT_VERSIONS`.
    ...
    **Amendment (2026-06-08 — packaged artifact health policy).** The admin/registry path must **not** run full `module:check` against installed modules and reject every out-of-spec artifact. Installed packages may be old but still usable, and the operator needs visibility rather than surprise breakage. The registry now runs a lightweight health check for managed installs under `<DATA_DIR>/modules`: missing/out-of-root required `manifest.ui` or `manifest.logic`, and browser UI imports that the runtime loader cannot resolve, are **errors** that mark that source `errored` and block enable/switch; optional-server/style drift, deprecated SDK names (`onActorChanged`, legacy hook names, `ModuleFoundryClient`), and non-browser private/external imports are **warnings** surfaced in admin validation diagnostics while the module remains loadable. This complements, but does not replace, contract-version discipline or true runtime-load failure reporting.
...
- [x] Fix stale server-route docs (`docs/MODULE_AUTHORING.md`) to current `ModuleRouteHandler` + `json()`/`error()` usage. — server-route section rewritten: static `apiRoutes` keyed by pattern, handler `(req, { params })`, `req.json()` is the request-body reader (not a response), responses via `json()`/`error()`, host access through `req.runtime` (no Foundry client). Also refreshed the adapter section (`initialize(runtime)` flat `ModuleRuntime`, no client on projection methods) and added an Assets section (`assetUrl()` / no binary imports / scoped CSS).
  ...
  **Amendment (2026-06-08 — manifest reference moved and realigned).** The original plan updated `docs/MODULE_AUTHORING.md` but left the full manifest reference as `src/modules/MODULE_MANIFEST.md`, which made authoring docs look like runtime source and let the reference drift. That file has been moved to `docs/MODULE_MANIFEST.md` and rewritten to match the completed ADR-0027 surface: SDK subpath entry points, `ModuleRuntime`/`req.runtime`, current adapter methods, `useSDK().events` instead of `onActorChanged`, managed UI loading through `/api/modules/:id/ui`, local-dev UI through `.managed/module-ui-registry.ts`, packager-managed Tailwind output, logger-only module diagnostics, and lightweight managed-artifact health warnings/errors. Live references now point at `docs/MODULE_MANIFEST.md`; historical ADR references were path-corrected without changing their historical intent.
...
- [x] Wire permission/trust + ownership enforcement; reads/writes fail closed; `commit` verifies per-op (decisions 9, 10). — route `DocumentStore` (`moduleDocumentServices.ts`): scoped reads fail closed when no subject resolves; writes require WRITEABLE (OWNER) ownership via `store.canReadDocument` (false for missing docs → ambiguous ownership blocks); the `{ access }` override trusts only `userId` (role re-derived from `userStore`, not caller-claimed); `commit` verifies every op before dispatching any. Trust/permission is install-time (no manifest document-scope gate — decision 10), so the live write gate is ownership. Covered by `src/tests/unit/sdk/module-document-store.test.ts`.
  ...
  **Amendment (2026-06-08 — checklist correction).** The completion note above is retained for history, but the corrected write model is narrower: route writes are bound to a real Foundry user session, the session user must match the route client's transport user, and write-time `{ access }` cannot name a different user. The live write gate is Foundry's user-bound socket plus the local ownership/readiness preflight; module trust/permission declarations are not document CRUD gates.
...
- [x] Implement `assetUrl()` and extend the asset route to serve local-dev modules; drop TSX asset imports (decision 27). — `buildModuleAssetUrl(moduleId, path)` helper + `SDKContextValue.assetUrl(path)` (bound to the surface's `moduleId`) → `/api/modules/<id>/assets/<path>`. The asset route (`createModuleRouter.ts`) now resolves installed **and** local-dev modules (`getLocalModulesDataDir()`) via `resolveModuleBaseDir`, so the same URL works in dev and packaged. dnd5e's TSX `import '../../assets/dnd5e.css'` dropped — the platform injects the declared `stylesheet` via `<link>` to that route.
  **Amendment (2026-09-04 — active-source assets and packageable CSS dependencies).** The original implementation made both roots reachable but selected them by a fixed managed-first filesystem order. When local development was the registry-selected source and an older managed package existed under the same module id, Core paired local code with managed assets. The asset route now resolves only the enabled lifecycle record's `activeSource`; it serves the local root during local development and the managed root for packaged operation, with no silent fallback to an inactive or disabled source. The stable `/api/modules/<id>/assets/<path>` contract is unchanged. `module:check` now also validates CSS copied directly from `assets/`: remote HTTP(S) dependencies, root paths outside the module asset API, paths escaping `assets/`, and missing referenced files block packaging. This closes the earlier gap where selector scoping could pass even though the resulting package depended on unavailable or CSP-blocked runtime assets.
...
- **Compatibility validation reports "valid" for runtime-incompatible installed modules** — see the addendum under decision 23. The admin view trusts the declared `apiContracts` semver, which we did not bump across breaking SDK surface changes, so a stale installed module reads as valid while its bundle won't load. Resolution levers (contract-version discipline + surfacing runtime load health) are recorded there for the post-stabilization follow-up; not yet implemented.
  ...
  **Amendment (2026-06-08 — admin-visible installed drift).** The "not yet implemented" note above still applies to marking an actual runtime import failure after the UI route attempts to load a bundle, but the registry no longer leaves statically detectable packaged drift invisible. Managed source states now carry artifact diagnostics, and the admin payload maps those diagnostics into the same validation panel used for manifest/core/contract findings. Warning-only drift remains loadable; blocking artifact errors reject enable/switch and return the source reason to the admin action.
  ...
  **Documentation-scope clarification (2026-09-02).** The abstract-module naming rule above applies to technical architecture, API, SDK, module-authoring, and operations documentation, where concrete local-development or managed-package examples can imply ownership or filesystem coupling that does not exist. The product README may name officially maintained system integrations and link to their independently managed repositories so users can discover supported integrations. Naming an integration there does not make it part of Core, conflate local and managed sources, or permit private host imports outside the SDK contract.

  **Amendment (2026-09-01 — source-state reconciliation and core fallback).** Configured testing found that the registry persisted the active source's enabled state in three places: the shared lifecycle record, `localEnabled`/`managedEnabled`, and `sourceStates`. Startup preferred the stale shared value while the split admin cards preferred a per-source value, so admin could show a local-development source enabled while Core refused its adapter. Local and managed per-source flags are now authoritative startup intent and every classification/runtime-failure mutation synchronizes all representations. Discovery clears stale local paths, an unavailable preferred source is replaced and relabelled with the source actually selected, and managed-package presence comes from the package directory rather than optional artifact metadata. A disabled, incompatible, or failed adapter records lifecycle health and executes no module code, but `getAdapter()` returns the internal `BaseSystemAdapter` fallback so core actor/combat APIs do not return HTTP 500 solely because optional system-module behavior is unavailable.

  **Operational clarification (2026-09-02):** Source disappearance does not copy enabled state between local and managed. If the remaining source is independently enabled and passes blocking validation, it becomes active without another admin switch; if it is disabled or blocked, Core uses the internal fallback and executes no module code. That fallback restores platform availability, not module functionality: system-specific normalization, derived computation, categorization, routes, and UI remain unavailable until a valid source is enabled. Lifecycle health, logs, and admin status remain the operator signal. This is an availability degradation policy and does not weaken Foundry document authorization or revive the service account as a user-write fallback.
...
- [x] Installed managed modules with survivable artifact drift remain loadable with admin warnings, while missing required packaged entries or unresolvable browser imports are admin-visible blocking errors. (Covered by `module-artifact-health.test.ts` and `module-registry-manager.test.ts`.)

...
33. **Two-layer delivery, with visual parity but differing isolation.** A module's Tailwind-derived CSS is produced in two distinct ways depending on context, accepted as a deliberate trade rather than forced into one mechanism:
    ...
    - **Packaged / installed (compiled, no source).** `module:package` compiles the module's own Tailwind into a **self-contained** `assets/<id>.css`, tree-shaken to the utilities that module actually uses, wrapped under `.sdk-module--<id>` by a PostCSS pass, and auto-declared as the manifest `stylesheet` (served via the decision-27 asset route). This is the portable, isolated form — it does not depend on the host scanning anything.
...
34. **A shared SDK theme supplies tokens by `@reference`; nothing is merged.** Modules resolve the platform's design tokens (custom fonts such as `--font-imfell`, the color palette, spacing) from a shared SDK theme entry (e.g. `@sheet-delver/sdk/theme.css`) via Tailwind v4 `@reference`. `@reference` is **token-resolution only — it emits no CSS** (unlike `@import "tailwindcss"`, which emits). Therefore each module compiles **independently** into its own file; the theme is never copied into a module and module stylesheets are never globbed together. The actual token *values* are still supplied at runtime by the host (`next/font` sets `--font-imfell` on the page); the module's compiled CSS only emits `font-family: var(--font-imfell)`. The one thing legitimately duplicated per module is Tailwind's preflight/base — this is accepted: it is small, scoped under `.sdk-module--<id>`, and harmless because only one module is enabled at a time.
...
- [ ] Styling parity restored — tracked in Phase 6.5 (morkborg authors in Tailwind utilities, which broke when it moved to gitignored `data/local/modules`; `module:check` is green because CSS is not gated).
...
- [ ] Extend the generated dynamic `@source` plugin (`start-server.ts` → `.managed/postcss-plugin.cjs`) to include the local-modules dir (`getLocalModulesDataDir()`, default `<DATA_DIR>/local/modules`) alongside the existing `<DATA_DIR>/modules` / `src/modules` globs — derived from the resolved data dir (no `data/` literal). Restores local-dev Tailwind for `morkborg`/`shadowdark`. Dynamic glob, no per-module list (decision 33, layer 1).
- [ ] Add a shared SDK theme entry (e.g. `@sheet-delver/sdk/theme.css`) exposing the host design tokens (fonts, palette, spacing) for `@reference`; confirm the runtime token vars (e.g. `--font-imfell`) are host-supplied so module CSS only emits `var(...)` (decision 34).
- [ ] Add a package-time Tailwind compile to the shared `build-config` / `package-module` path: compile each module's `src/**` utilities → `assets/<id>.css`, tree-shaken, `@reference`-ing the shared theme, wrapped under `.sdk-module--<id>` by a PostCSS pass; auto-declare it as the manifest `stylesheet` (decision 33, layer 2).
- [ ] Decide preflight handling — accept per-module duplication, scoped under the surface root (decision 34).
- [ ] Update the `module:check` style-scope lint to accept compiler-generated scoped output while still rejecting global leaks in hand-authored CSS (decision 35).
- [ ] Keep hand-authored scoped CSS (`dnd5e`) a valid path; the Tailwind compile is automatic but a module may still ship its own stylesheet (decision 32).
- [ ] Conform `init-module` (decision 29) to the chosen authoring model: the scaffolded `Sheet.tsx` models Tailwind-in-JSX (utility classes) as the happy path; reconcile the scaffold's hand-authored `assets/styles.css` + manual `stylesheet:` declaration with the package-time compile that auto-produces and auto-declares `assets/<id>.css` (the scaffolded file becomes the author's *optional* custom CSS fed into the compile, not a competing declaration). Scaffold a module CSS entry only if the compile is entry-driven (`@reference` the shared theme) rather than convention-driven; either way, keep the hand-authored semantic-CSS path documented as the alternative. `tsconfig.json` and the CI workflow (`module:check` + `module:package`) are unaffected.
- [ ] Prove it on `morkborg`: confirm `assets/morkborg.css` contains only morkborg's utilities, scoped; the running sheet matches its historical visual in both `npm run dev` and a packaged install.
